...
//check ig loged
if(!isset($_SESSION['user']['logged']) OR !$_SESSION['user']['logged']==true){
    // Render HTML output 
    @header('Content-type: text/html; charset=utf-8');  
    exit('<script>alert("Not alloyed!")</script>');
}
header("Content-Type: text/html; charset=utf-8\n");  
...
foreach($files as $file){  
    ...
    $image_info = getimagesize($server_path.$thumbSrc);  
    $_w = $image_info[0];  
    $_h = $image_info[1]; 
